...
public class WkController {
	...
	public String insert(Model model, @RequestParam(defaultValue="1") String currentPage,
			@RequestParam(defaultValue="1") String currentSitePage, String msg) {
//		System.out.println("redirect에서 받은 msg : " + msg); // null
	...
	public Map<String,Object> deletePostMy(@RequestBody Map<String, Object> map,
			Model model) {
		logger.info("empNumMyDel : " + map.get("empNum"));
		...
		if(resultSel == 0) {//근무 테이블에 사원 존재X
			int resultDel = this.wkService.deleteEmpMy(empNum); 
