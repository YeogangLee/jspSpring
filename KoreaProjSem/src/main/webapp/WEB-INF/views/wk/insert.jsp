...
// 		alert("varSiteNum : " + varSiteNum + ", varSiteNm : " + varSiteNm);
	...
	$(".delEmp").on("click",function(e){
		...
		var empName = e.target.parentElement.innerText.trim();
		var empNo = e.target.parentElement.previousElementSibling.innerHTML;
		
		if(confirm("사원 : " + empName + "(" + empNo + ")" + "의 정보를 삭제하시겠습니까?")) {
			...
			$.ajax({
				...
				,chche:false
				,success:function(data){
					//선생님 삭제 메서드 이용시(,url:"/wk/delete")
	// 				console.log("empNum(returned) : " + data.empNum);
	// 				if(data.empNum > 0){//삭제성공
	// 					parent.location.reload(); //페이지 새로고침
	// 				}else{
	// 					alert("삭제할 수 없는 데이터입니다.");
	// 				}
					
					//내 숙제 삭제 메서드 이용시(,url:"/wk/myDelete")
					if(data.msg == "성공"){//삭제성공
						alert("사원 데이터를 삭제했습니다.");
...
function fn_siteReg(){
	//"/site/popUp/insert" 이렇게 경로를 3가지 이용하면, tiles css가 적용되지 않는다.
	//tiles에서 경로를 2가지로 지정했기 때문이다.
