...
//@RequestMapping("/item")
//@Controller
public class ItemController_old {
	
	private static final Logger logger = LoggerFactory.getLogger(ItemController_old.class);
	...
	public String registerPost(MultipartFile picture, @ModelAttribute Item item, Model model) {
		...
		//파일은 picture 파라미터로도 넘어오지만
		//아이템vo의 필드셋의 picture와도 매치되어, 아이템vo에도 set되어 있어서 get 가능
		logger.info("파일명 : " + file.getOriginalFilename());
		//파일명을 item vo의 pictureURL에 넣어준다.
		//단 업로드 후에 넣어준다, 왜냐하면 업로드 과정에서 중복 방지를 위해 uuid를 이용하여 파일명이 변경되기 때문이다.
...
//			picturesUrl[cnt++] = uploadFile(multipartFile); //후위 연산자, 대입 후 1 증가
		...
		for(int j=0; j < picturesUrl.length; j++) {
			logger.info(j + "번째 이미지 업로드 최종경로  : "+picturesUrl[j]);
		}
		...
		int itemId = itemSerivce.create(item);
